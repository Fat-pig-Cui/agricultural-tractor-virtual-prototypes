...
\pubyear{2026}
\copyrightyear{2026}
\Title{A Feasibility-First Virtual Benchmark for Causal Energy Management in a Hybrid Agricultural Tractor}
...
\dateaccepted{}
\datepublished{}
\abstract{Virtual fuel-saving claims for hybrid agricultural tractors are inseparable from terminal-energy accounting, component-map assumptions, and online feasibility. This study presents a feasibility-first benchmark for causal energy management in a parameterized OS-ECVT virtual prototype. It compares map-aware equivalent-consumption minimization (ECMS), a public-phase policy, deterministic model predictive control (MPC), scenario MPC, and terminal-set safeguarded scenario MPC under one information set, one applied-actuator model, and one out-of-loop baseline. The 300 kW virtual platform imposes a 60 kW stable engine-power floor, a 40 kW s$^{-1}$ applied ramp, 150 kW motor and battery-bus limits, proxy MG limits, and a terminal SOC error no larger than $10^{-4}$. Under a literature-shaped map, terminal-set MPC was certified on 20/20 stochastic paths with zero external fallback and zero terminal-set violations, but increased paired fuel use by 0.704\% (95\% CI: 0.653\%--0.756\%). An independent cross-cycle audit found the same feasibility--fuel trade-off on deterministic and stochastic agricultural cycles, while a high-variability cycle exceeded the declared battery-power envelope and was reported as invalid rather than repaired. A continuous seven-level virtual map sweep showed that ECMS savings changed from 12.543\% to a 1.075\% fuel increase as the declared low-load efficiency rose from 0.08 to 0.35; terminal-set MPC remained fuel-inferior throughout. Frozen storage-capacity and causal-forecast sensitivity audits preserved strict feasibility but did not recover a positive MPC fuel result. The separately reported full-information oracle is excluded from this abstract and from all causal-controller statistics. The benchmark therefore identifies conditional feasibility, map sensitivity, and accounting boundaries rather than a generic MPC fuel advantage.}
\keyword{hybrid agricultural tractor; energy management; terminal state of charge; ECMS; model predictive control; component-map sensitivity; virtual powertrain}
\begin{document}
...
This article asks two engineering questions. First, when the information set, terminal energy, actuator limits, and baseline are fixed, which causal controller remains both feasible and fuel-competitive? Second, does this answer persist when a literature-shaped engine-efficiency map is contrasted with a deliberately steep low-load sensitivity map on the same virtual hardware? The purpose of the second question is not to claim a physical map for a target tractor. It is to determine whether a controller ranking survives a transparent change in a declared virtual-model assumption.

The study is organized around an auditable comparison rather than a headline fuel-saving result. All online methods share the same virtual powertrain, terminal-energy governor, hard limits, stochastic paths, and out-of-the-loop baseline. Development seeds are separated from test seeds wherever a parameter or horizon is selected. These choices allow a negative result to be reported meaningfully: a controller can be feasible yet fuel-inferior under a stated map, and an attractive offline trajectory can remain a diagnostic rather than evidence of causal performance.
...
This work addresses that comparison gap with a shared strict protocol for map-aware ECMS, a public-phase causal policy, deterministic MPC, scenario MPC, and terminal-set safeguarded scenario MPC. The terminal-set construction exposes safety-filter activations rather than silently relying on a controller-external fallback. The paper also reports a full-information oracle only as an explicitly idealized headroom diagnostic. The scope is deliberately limited to simulation: the component maps are defined virtual-model inputs, not claims of field, vehicle, or bench measurement.

For an energy-systems audience, the central contribution is therefore an auditable accounting protocol as much as a controller comparison. The protocol keeps the plant, terminal SOC rule, applied engine dynamics, and baseline identical across methods, while recording every internal feasibility intervention. This prevents an apparently favorable fuel number from being produced by unequal residual battery energy or by an unreported post-optimization repair. The resulting negative MPC finding is useful as a conditional energy-management boundary: it identifies the declared map, information set, and finite candidate structure for which feasibility was obtained without a fuel advantage.

The contributions are as follows: (i) a feasibility-first, reproducible OS-ECVT virtual benchmark with a strict terminal-energy and hard-constraint rule; (ii) a common causal comparison of map-aware ECMS, a public-phase policy, deterministic MPC, scenario MPC, and terminal-set safeguarded scenario MPC; (iii) development/test separation for the only tuned ECMS parameter; (iv) an auditable conditional recursive-feasibility construction that separates internal safety-filter interventions from fallback applied outside the controller; (v) an explicitly labeled map-mechanism sensitivity on the same 300 kW virtual hardware; (vi) a predeclared horizon--preview mechanism map with an additional development/holdout split; and (vii) an independent cross-cycle demand-envelope audit that reports invalid high-variability paths instead of converting them into favorable fuel statistics. The resulting paper is a boundary study, not a claim of generic MPC fuel superiority. Offline dynamic programming is not included in the online statistics because it has access to future demand.
...
To test the boundary of the declared platform rather than silently exclude difficult demand, an independent cross-cycle audit used 20 seeds (2026--2045), the literature-shaped map, the common 18 kW accessory load, terminal-SOC tolerance $10^{-4}$, and no offline SOC reference. It evaluated the deterministic agricultural work cycle, the stochastic agricultural cycle, and a high-variability cycle with the engineering OOL, map-aware ECMS, and terminal-set MPC. The audit was not used to retune a controller. A cycle was reported as invalid when the common battery-power or other hard constraint failed; invalid paths were not converted into fuel-saving pairs.
